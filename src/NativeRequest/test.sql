SELECT
	NULL AS product_id,
	sco_prio.country_id AS country_id,
	NULL AS theme_id,
	NULL AS criterion_id,
	7 AS profile_id,
	SUM(sco_prio.score*sco_prio.value*sco_prio.transparency)
		/ SUM(sco_prio.value*sco_prio.transparency) AS value,
	SUM(sco_prio.value*sco_prio.transparency) 
		/ (SELECT SUM(value) AS value_sum FROM priority WHERE profile_id = 7) AS coverage,
...
FROM (
	...
	FROM priority prio
		JOIN (	
			...
			WHERE
				pri.profile_id = 7 AND ra.country_id = 692
			GROUP BY 
				ra.country_id,
				pri.id
		) AS sco
